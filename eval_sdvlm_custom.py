# -*- coding: utf-8 -*-
"""
Created on Tue Oct 21 08:33:01 2025
...
def eval_sdvlm(args):
    ...
    # === Dataset（改為：單張自訂圖片）===
    # 保持後續流程不變，所以用 DataFrame 包一筆資料，欄位結構仿照原本 parquet
    print("📂 Using your custom single image instead of test.parquet")
    prompt = args.prompt if args.prompt else "<image>\nPlease describe the scene."
    single_row = {
        "image_path": args.image,                # ← 你的圖檔路徑
        "image_base64": None,                    # 保留欄位，但不用
        "image_size": None,                      # 會在開圖後由 PIL 填入/使用
        "image_mode": None,                      # 會在開圖後由 PIL 填入/使用
        "conversations": [
            {"from": "human", "value": prompt},
            {"from": "gpt", "value": ""}         # gt 可留空
        ],
        "type": "custom"
    }
    ...
    # === 推論迴圈（保留一樣的寫法，只是資料是 1 筆）===
    for i in tqdm(range(len(data))):
        sample = data.iloc[i]
        prompt = sample["conversations"][0]["value"]
        gt = sample["conversations"][1]["value"]

        # ⬇️ 關鍵改動：從 image_path 讀圖（若你真的想用 base64 也可保留原分支）
        if isinstance(sample.get("image_path", None), str) and os.path.exists(sample["image_path"]):
            image = Image.open(sample["image_path"])
        else:
            image = base64_to_pil(sample.get("image_base64", ""))

        if image is None:
            print("❌ 圖片讀取失敗，請確認 --image 路徑")
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-samples", type=int, default=1)  # 單張測試固定 1
    parser.add_argument("--image", type=str, default="/home/itris3/SD-VLM/cup.jpg",
                        help="Path to your test image.")
    parser.add_argument("--prompt", type=str, default= "<image>\nIf the far between red cup and  grey cup?",
